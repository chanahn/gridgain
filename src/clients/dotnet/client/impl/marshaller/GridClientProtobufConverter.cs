...
namespace GridGain.Client.Impl.Marshaller {
    ...
    internal static class GridClientProtobufConverter {
        ...
        private static ProtoNodeBean WrapNode(GridClientNodeBean node) {
            ...
            if (node.ExternalAddresses != null)
                foreach (String addr in node.ExternalAddresses)
                    builder.AddExternalAddress(addr);

            if (node.InternalAddresses != null)
                foreach (String addr in node.InternalAddresses)
                    builder.AddInternalAddress(addr);
