...
namespace GridGain.Client.Util {
    ...
    internal static class GridClientUtils {
        ...
        private static readonly byte[] GUID_CONV = new byte[] { 6, 7, 4, 5, 0, 1, 2, 3, 15, 14, 13, 12, 11, 10, 9, 8 };
        ...
        public static byte[] ToBytes(Guid id) {
            ...
            byte[] java = new byte[GUID_CONV.Length];

            // Convert to Java format
            for (int i = 0; i < GUID_CONV.Length; i++)
                java[i] = cs[GUID_CONV[i]];
         ...
         */
        public static Guid BytesToGuid(byte[] java) {
            byte[] cs = new byte[GUID_CONV.Length];

            // Convert from Java format
            for (int i = 0; i < GUID_CONV.Length; i++)
                cs[GUID_CONV[i]] = java[i];
