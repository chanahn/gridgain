 ...
 * <li>Join timeout (see {@link #setJoinTimeout(int)})</li>
...
public class GridTcpDiscoverySpi extends GridSpiAdapter implements GridDiscoverySpi, GridTcpDiscoverySpiMBean {
    ...
    /** Default socket operations timeout in milliseconds (value is <tt>1000</tt>). */
    public static final int DFLT_SOCK_TIMEOUT = 1000;

    /** Default timeout for receiving message acknowledgement in milliseconds (value is <tt>2000</tt>). */
    public static final int DFLT_ACK_TIMEOUT = 2000;

    /** Default timeout for joining topology (value is <tt>0</tt>). */
    public static final int DFLT_JOIN_TIMEOUT = 0;
    ...
    /** Join timeout. */
    private int joinTimeout = DFLT_JOIN_TIMEOUT;
    ...
    /** {@inheritDoc} */
    @Override public int getJoinTimeout() {
        return joinTimeout;
    }
    ...
    private class MessageWorker extends GridSpiThread {
        ...
        private void processJoinRequestMessage(GridTcpDiscoveryJoinRequestMessage msg) {
            ...
            if (isLocalNodeCoordinator()) {
                ...
                if (existingNode != null) {
                    ...
                    // New node is joining, assign order.
                    long topVer = ring.incrementTopologyVersion();
                ...
                synchronized (mux) {
                    if (spiState == CONNECTING) {
                        if (topStore != null) {
                            ...
                            assert node.order() == msg.topologyVersion();

                            // Artificial "restore" topology to order nodes in the ring.
                            ring.restoreTopology(ring.remoteNodes(), node.order());
        ...
        private void processNodeAddFinishedMessage(GridTcpDiscoveryNodeAddFinishedMessage msg) {
            ...
            if (msg.verified() && !locNodeId.equals(nodeId) && spiStateCopy() == CONNECTED && fireEvent) {
                ...
                notifyDiscovery(EVT_NODE_JOINED, node.order(), node);
            ...
            if (msg.verified() && locNodeId.equals(nodeId) && spiStateCopy() == CONNECTING) {
                ...
                if (recon)
                    notifyDiscovery(EVT_NODE_RECONNECTED, locNode.order(), locNode);
                else
                    recon = true;
            }
        ...
        @Override protected void body() throws InterruptedException {
            try {
                ...
                try {
                    ...
                    writeToSocket(sock, new GridTcpDiscoveryHandshakeResponse(locNodeId, locNode.order()));
