...
public class GridJettyRestHandler extends AbstractHandler {
    ...
    private void processRequest(String action, HttpServletRequest req, HttpServletResponse res) {
        ...
        GridRestRequest cmdReq = new GridRestRequest(cmd, action, parameters(req));
        ...
        try {
            json = JSONSerializer.toJSON(cmdRes);
        }
        ...
        try {
            ...
            if (log.isDebugEnabled())
                log.debug("Processed HTTP request [action=" + action + ", jsonRes=" + cmdRes + ", req=" + cmdReq + ']');
        }
        catch (IOException e) {
