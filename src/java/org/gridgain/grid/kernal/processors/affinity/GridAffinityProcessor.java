...
public class GridAffinityProcessor extends GridProcessorAdapter {
    ...
    private final ConcurrentMap<String,
        GridTuple4<GridCacheAffinityMapper, GridCacheAffinity, GridException, CountDownLatch>> affMap =
    ...
    private <K> Map<GridRichNode, Collection<K>> keysToNodes(@Nullable final String cacheName,
        ...
        Collection<GridRichNode> cacheNodes = F.viewReadOnly(
        ...
        if (old != null) {
            ...
            if (old.get2() != null)
                return affinityMap(cacheName, old.get1(), old.get2(), keys, cacheNodes); // Only pass cache nodes.
        }

        int max = ERROR_RETRIES;
        int cnt = 0;

        Iterator<GridRichNode> it = cacheNodes.iterator();
    ...
    private GridTuple2<GridCacheAffinityMapper, GridCacheAffinity> affinityFromNode(@Nullable String cacheName,
        ...
        GridTuple3<GridAffinityMessage, GridAffinityMessage, GridException> t = ctx.task()
            .execute(affinityTask(cacheName), n, 0, null, /*system pool*/sys).get();
