...
class GridNioSslHandler extends ReentrantLock {
        ...
        while (loop) {
            switch (handshakeStatus) {
                ...
                case NEED_WRAP: {
                    ...
                    SSLEngineResult result = sslEngine.wrap(handshakeBuf, outNetBuf);
                    ...
                    if (log.isDebugEnabled())
                        log.debug("Wrapped handshake data [status=" + result.getStatus() + ", handshakeStatus=" +
    ...
    void encrypt(ByteBuffer src) throws SSLException {
        ...
        while (src.hasRemaining()) {
            if (src.remaining() > (outNetBuf.capacity() - outNetBuf.position()) / 2) {
                outNetBuf = GridNioSslFilter.expandBuffer(outNetBuf, src.capacity() * 2);
            ...
            SSLEngineResult result = sslEngine.wrap(src, outNetBuf);

            if (log.isDebugEnabled())
                log.debug("Encrypted data [status=" + result.getStatus() + ", handshakeStaus=" +
                    result.getHandshakeStatus() + ", ses=" + ses + ']');

            if (result.getStatus() == SSLEngineResult.Status.OK) {
                if (result.getHandshakeStatus() == NEED_TASK)
                    runTasks();
            } else
                throw new SSLException("Failed to encrypt data (SSL engine error) [status=" + result.getStatus() +
                    ", handshakeStatus=" + result.getHandshakeStatus() + ", ses=" + ses + ']');
        }
    ...
    GridNioFuture<?> deferredWrite(ByteBuffer buf) {
        ...
        GridNioEmbeddedFuture<Object> fut = new GridNioEmbeddedFuture<Object>();
