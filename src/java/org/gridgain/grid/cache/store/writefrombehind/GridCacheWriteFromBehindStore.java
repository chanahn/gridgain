 ...
 */
public class GridCacheWriteFromBehindStore<K, V> implements GridCacheStore<K, V>, GridCacheWriteFromBehindStoreMBean {
    ...
    public static final int DFLT_MAX_CACHE_SIZE = 10 * 1024;

    /** Default count of worker threads. */
    public static final int DFLT_WORKER_THREAD_CNT = 2;

    /** Default value for write cache flush frequency is 5 seconds. */
    public static final int DFLT_CACHE_FLUSH_FREQ = 5000;

    /** Default value for maximum batch size */
    public static final int DFLT_BATCH_SIZE = 128;
    ...
    private int cacheMaxSize = DFLT_MAX_CACHE_SIZE;
    ...
    private int flushThreadCnt = DFLT_WORKER_THREAD_CNT;

    /** Cache flush frequency. All pending operations will be performed in not less then this value ms. */
    private int cacheFlushFreq = DFLT_CACHE_FLUSH_FREQ;
    ...
    /** Variable for counting cache overflows. */
    ...
    /** {@inheritDoc} */
    @Override public int getFlushSize() {
        return cacheMaxSize;
    }
    ...
    /** {@inheritDoc} */
    @Override public int getFlushThreadCount() {
     ...
     */
    public void setFlushFrequency(int cacheFlushFreq) {
        this.cacheFlushFreq = cacheFlushFreq;
    }

    /** {@inheritDoc} */
    @Override public int getFlushFrequency() {
     ...
     */
    @Override public int getCacheOverflowCounter() {
        return cacheOverflowCntr.get();
    }

    /**
     * @return Count of entries in {@link ValueStatus#RETRY} state.
     */
    @Override public int getRetryEntriesCount() {
    ...
    private void updateCache(K key, @Nullable V val, StoreOperation operation) throws GridInterruptedException {
        ...
        if (writeCache.sizex() > cacheCriticalSize)
            ...
            flushSingleValue();
        else if (writeCache.sizex() > cacheMaxSize)
            wakeUp();
    }
    ...
    private class Flusher extends GridWorker {
        ...
        private void awaitOperationsAvailable() throws InterruptedException {
            ...
            try {
                do {
                    // In any case we should wait at most cacheFlushFreq ms.
                    if (writeCache.sizex() <= cacheMaxSize)
