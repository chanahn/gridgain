...
public class GridConcurrentHashMap<K, V>
     ...
     */
    private transient final GridLongAdder counter;
    ...
    static final class Node {
        ...
        final void tryAwaitLock(Node[] tab, int i) {
            if (tab != null && i >= 0 && i < tab.length) { // bounds check
                int r = GridThreadLocalRandom.current().nextInt(); // randomize spins
     ...
     */
    public GridConcurrentHashMap() {
        this.counter = new GridLongAdder();
    }
     ...
     */
    public GridConcurrentHashMap(int initialCapacity) {
     ...
     * @param m the map
     */
    public GridConcurrentHashMap(Map<? extends K, ? extends V> m) {
        this.counter = new GridLongAdder();
     ...
     * @since 1.6
     */
    public GridConcurrentHashMap(int initialCapacity, float loadFactor) {
    ...
    static abstract class ViewIterator<K,V> extends InternalIterator {
        final GridConcurrentHashMap<K, V> map;
        ViewIterator(GridConcurrentHashMap<K, V> map) {
            super(map.table);
            this.map = map;
        }
    ...
    static final class KeyIterator<K,V> extends ViewIterator<K,V>
        ...
        KeyIterator(GridConcurrentHashMap<K, V> map) { super(map); }
    ...
    static final class ValueIterator<K,V> extends ViewIterator<K,V>
        ...
        ValueIterator(GridConcurrentHashMap<K, V> map) { super(map); }
    ...
    static final class EntryIterator<K,V> extends ViewIterator<K,V>
        ...
        EntryIterator(GridConcurrentHashMap<K, V> map) { super(map); }
    ...
    static final class SnapshotEntryIterator<K,V> extends ViewIterator<K,V>
        implements Iterator<Map.Entry<K,V>> {
        SnapshotEntryIterator(GridConcurrentHashMap<K, V> map) { super(map); }
    ...
    static final class WriteThroughEntry<K,V> extends MapEntry<K,V>
        ...
        final GridConcurrentHashMap<K, V> map;
        WriteThroughEntry(K key, V val, GridConcurrentHashMap<K, V> map) {
            super(key, val);
            this.map = map;
        }
    ...
    static abstract class MapView<K, V> {
        final GridConcurrentHashMap<K, V> map;
        MapView(GridConcurrentHashMap<K, V> map)  { this.map = map; }
    ...
    static final class KeySet<K,V> extends MapView<K,V> implements Set<K> {
        KeySet(GridConcurrentHashMap<K, V> map)   { super(map); }
    ...
    static final class Values<K,V> extends MapView<K,V>
        ...
        Values(GridConcurrentHashMap<K, V> map)   { super(map); }
    ...
    static final class EntrySet<K,V> extends MapView<K,V>
        ...
        EntrySet(GridConcurrentHashMap<K, V> map) { super(map); }
    ...
    private void readObject(java.io.ObjectInputStream s)
        ...
        UNSAFE.putObjectVolatile(this, counterOffset, new GridLongAdder());
    ...
    static {
        ...
        try {
            UNSAFE = getUnsafe();
            Class<?> k = GridConcurrentHashMap.class;
