...
object ScalarContinuationExample {
    def main(args: Array[String]) {
        scalar {
            ...
            val fib = grid$.call(UNICAST, new FibonacciClosure(thisNodeId), N, p)
...
class FibonacciClosure (
    ...
    @Nullable override def apply(num: Long): BigInteger = {
        if (fut1 == null || fut2 == null) {
            ...
            // Predicate that excludes node with id passed in constructor.
            val p = (n: GridRichNode) => grid$.remoteNodes().isEmpty || n.id != excludeNodeId

            // If future is not cached in node-local store, cache it.
            // Note recursive grid execution!
            if (fut1 == null)
                fut1 = store.addIfAbsent(n - 1, g.callAsync(UNICAST, new FibonacciClosure(excludeNodeId), n - 1, p))

            // If future is not cached in node-local store, cache it.
            if (fut2 == null)
                fut2 = store.addIfAbsent(n - 2, g.callAsync(UNICAST, new FibonacciClosure(excludeNodeId), n - 2, p))
