...
package org.gridgain.examples.cache.dataload;
...
public class GridCacheHdfsDataLoaderExample {
    ...
    private static void loadData(Grid grid) throws Exception {
        ...
        X.println(">>>", ">>> Using data loader: " + ldr, ">>>");
    ...
    private static void walk(FileSystem fs, Path path, GridDataLoader<UUID, Person> ldr, Queue<Future<?>> futs)
        ...
        for (FileStatus s : fs.listStatus(path)) {
            if (s.isDir())
                ...
                futs.add(exec.submit(new HdfsWorker<UUID, Person>(fs, s.getPath(), ldr)));
        }
    }
    ...
    private static Path path(FileSystem fs) {
        ...
        return fs instanceof LocalFileSystem ?
            new Path(X.getSystemOrEnv(GridSystemProperties.GG_HOME, ""), LOCAL_DIR) :
            new Path(HDFS_DIR);
    }
     ...
     * onto data grid.
     */
    private static class HdfsWorker<K, V> implements Callable<Object> {
        /** Path to process. */
        private Path path;

        /** Loader. */
        private GridDataLoader<K, V> ldr;
         ...
         * @param ldr Loader.
         */
        private HdfsWorker(FileSystem fs, Path path, GridDataLoader<K, V> ldr) {
            assert path != null;
            assert ldr != null;
