...
package org.gridgain.examples.cache.query.snowflake;
 ...
 * your whole data grid, thus querying your in memory data without any limitations.
 * <p>
...
public class GridSnowflakeSchemaExample {
    ...
    private static void populateDimensions() throws GridException {
        GridCache<Integer, Object> cache = G.grid().cache("replicated");
    ...
    private static void populateFacts() throws GridException {
        GridCache<Integer, Object> dimCache = G.grid().cache("replicated");
        GridCache<Integer, Object> factCache = G.grid().cache("partitioned");
        ...
        for (int i = 0; i < 100; i++) {
            ...
            DimStore store = F.rand(stores.values());
            DimProduct prod = F.rand(prods.values());
    ...
    private static void queryStorePurchases() throws GridException {
        GridCache<Integer, FactPurchase> factCache = G.grid().cache("partitioned");

        // All purchases for store1.
        // ========================

        // Create cross cache query to get all purchases made at store1.
        GridCacheQuery<Integer, FactPurchase> storePurchases = factCache.createQuery(
            SQL,
            FactPurchase.class,
        ...
        printQueryResults("All purchases made at store1:",
            storePurchases.queryArguments("Store1").execute(G.grid()).get());
    }
    ...
    private static void queryProductPurchases() throws GridException {
        ...
        DimProduct p1 = F.rand(prods.values());
        DimProduct p2 = F.rand(prods.values());
        DimProduct p3 = F.rand(prods.values());

        X.println("IDs of products [p1=" + p1.getId() + ", p2=" + p2.getId() + ", p3=" + p3.getId() + ']');

        // Create cross cache query to get all purchases made at store2
        // for specified products.
        GridCacheQuery<Integer, FactPurchase> prodPurchases = factCache.createQuery(
            SQL,
            FactPurchase.class,
        ...
        printQueryResults("All purchases made at store2 for 3 specific products:",
            prodPurchases.queryArguments("Store2", p1.getId(), p2.getId(), p3.getId()).execute(G.grid()).get());
    }
