...
public class GridClientApiExample {
    ...
    /**
     * Runs client cache example and client compute example.
     *
    ...
    private static void clientComputeExample() throws GridClientException {
        ...
        try {
            ...
            try {
                ...
                X.println(">>> First log lines:", log.toArray());
                X.println("<<< End of log.");

                // Log entries may be fetched asynchronously.
                GridClientFuture<List<String>> futLog = prj.logAsync(2, 9);

                X.println(">>> First log lines fetched asynchronously: ", futLog.get().toArray());
                X.println("<<< End of log.");

                // Log file name can also be specified explicitly.
                log = prj.log("work/log/gridgain.log", 2, 9);

                X.println(">>> First log lines from log file work/log/gridgain.log:",  log.toArray());
                X.println("<<< End of log.");
